# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'IG1.ui'
#
# Created by: PyQt5 UI code generator 5.15.4
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_IG1_Bienvenida(object):
    def setupUi(self, IG1_Bienvenida):
        IG1_Bienvenida.setObjectName("IG1_Bienvenida")
        IG1_Bienvenida.resize(493, 348)
        IG1_Bienvenida.setLayoutDirection(QtCore.Qt.LeftToRight)
        IG1_Bienvenida.setStyleSheet("background-color: rgb(255, 164, 236);")
        self.botonUsuario1 = QtWidgets.QPushButton(IG1_Bienvenida)
        self.botonUsuario1.setGeometry(QtCore.QRect(180, 180, 121, 41))
        self.botonUsuario1.setLayoutDirection(QtCore.Qt.LeftToRight)
        self.botonUsuario1.setStyleSheet("background-color: rgb(80, 190, 255);\n"
"font: 14pt \"Terminal\";\n"
"color: rgb(255, 255, 255);")
        self.botonUsuario1.setObjectName("botonUsuario1")
        self.LabelBienvenida = QtWidgets.QLabel(IG1_Bienvenida)
        self.LabelBienvenida.setGeometry(QtCore.QRect(110, 70, 291, 41))
        self.LabelBienvenida.setStyleSheet("color: rgb(255, 255, 255);\n"
"font: 20pt \"Viner Hand ITC\";")
        self.LabelBienvenida.setObjectName("LabelBienvenida")
        self.label = QtWidgets.QLabel(IG1_Bienvenida)
        self.label.setGeometry(QtCore.QRect(0, 260, 131, 81))
        self.label.setText("")
        self.label.setPixmap(QtGui.QPixmap(":/prefijoNuevo/seña.png"))
        self.label.setScaledContents(True)
        self.label.setObjectName("label")

        self.retranslateUi(IG1_Bienvenida)
        QtCore.QMetaObject.connectSlotsByName(IG1_Bienvenida)

    def retranslateUi(self, IG1_Bienvenida):
        _translate = QtCore.QCoreApplication.translate
        IG1_Bienvenida.setWindowTitle(_translate("IG1_Bienvenida", "Form"))
        self.botonUsuario1.setText(_translate("IG1_Bienvenida", "USUARIO 1"))
        self.LabelBienvenida.setText(_translate("IG1_Bienvenida", "Selecciona un usuario"))
import seña1_rc


if __name__ == "__main__":
    import sys
    app = QtWidgets.QApplication(sys.argv)
    IG1_Bienvenida = QtWidgets.QWidget()
    ui = Ui_IG1_Bienvenida()
    ui.setupUi(IG1_Bienvenida)
    IG1_Bienvenida.show()
    sys.exit(app.exec_())
